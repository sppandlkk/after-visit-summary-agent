...
def build_avs(patient: Dict[str,Any],
    ...
    provenance = {"patient": [], "visit": [], "history": [], "diagnoses_and_issues": [], "treatment_plan": [], "follow_up": []}
    ...
    if birth != "unknown": provenance["patient"].append("FHIR:Patient.birthDate")
    gender = patient.get("gender","unknown")
    if gender != "unknown": provenance["patient"].append("FHIR:Patient.gender")
    ...
    history_text = ""
    if cond_summ:
        history_text += "Conditions: " + "; ".join(sorted(set([x for x in cond_summ if x])))
    if med_hist:
        if history_text: history_text += " | "
        history_text += "Medications: " + "; ".join(sorted(set([x for x in med_hist if x])))
    if history_text:
    ...
    for r in med_requests:
        if isinstance(r, dict):
            ...
            meds_now.append(r.get("resource",{}).get("medicationCodeableConcept",{}).get("text","" ))
    rag_context = []
    for med in meds_now:
        if not med: continue
        for res in kb.retrieve(med, k=k):
            rag_context.append(f"# {med} | Source: {res['path']}\n{res['snippet']}")
    ...
    for line in transcript_text.splitlines():
        low = line.lower()
        if "follow-up" in low or "follow up" in low or "see you in" in low or "return in" in low:
            follow = line.strip()
